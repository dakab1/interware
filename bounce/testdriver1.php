...
if($_GET['eml']){
    ...
    $email_file = "/home/visionwa/mail/".$_GET['eml'];
    //$bounce = file_get_contents($email_file);
    
    if ($handle = fopen($email_file,"r")) {
        
        echo "<span style='color:red; text-weight:bolder'>Openned " . $email_file . "</span>";
        if (!$bounce = fread($handle, filesize($email_file))) {
            
            $bounce =  "Failed to read contents";
            $bounce .= "filesize is " . filesize($email_file);
            
        }
        fclose($handle);
        
        echo "<div style='border: green 1px solid;'><u>Contents</u><p>$bounce</p></div>";
        
    } else {
        
        echo "<span style='color:red; text-weight:bolder'>Failed to open " . $email_file . "</span>";
    }

    echo "<div style='border:3px solid red'>|$bounce|</div>"; //debug
    
    if($handle = opendir("/home/visionwa/mail/")) { //debug

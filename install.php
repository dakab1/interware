...
if (isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        ...
        case "Install":
            ...
            if (!empty(mysqli_connect_error())) {
                ...
                if (!$db->multi_query($install_sql)) {
                    ...
                    $db->multi_query("INSERT INTO `user` (
                                        ...
                                        VALUES (
                                        NULL ,  'Default Campaign',  '" . date("Y-m-d H:i:s") . "',  '1',  '1',  NULL, NULL
                                        );");    
                    if ($db->error) {
                        $error=true;
            ...
            if (file_exists($domain_setup_document_root."/modules/config.php")) {

                if (!rename($domain_setup_document_root."/modules/config.php", "modules/config_" . date("YmdHis") . ".php")) {
                    $message = "<span style='color:red'>Failed to backup current $domain_setup_document_root/modules/config.php</span>";
                    $error = true;
                }
            }
    ...
    <body>
        ...
        <div>
            <form class="install_form" id="installForm" method="POST">
                ...
                <span><label>Document root</label><input class="required" type="text" name="domain_setup_document_root" value="<?= ($domain_setup_document_root ? $domain_setup_document_root : $_SERVER['DOCUMENT_ROOT'] . str_replace("/install.php", "", $_SERVER['PHP_SELF'])) ?>" />***</span><br/>
